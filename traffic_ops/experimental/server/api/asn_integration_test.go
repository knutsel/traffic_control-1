...
func TestCreateAsn(t *testing.T) {
	asnJson := `{
		"asn": 33099,  
		"_links": {
    		"cachegroup": {
	...
	reader = strings.NewReader(asnJson)

	asnUrl := fmt.Sprintf("%s/%s/asn", server.URL, api.API_PATH)
	request, err := http.NewRequest("POST", asnUrl, reader)
	...
	if res.StatusCode != 201 {
		t.Errorf("Success expected: %d", res.StatusCode, res)
	}
}
...
// func TestCreateAsn(t *testing.T) {
// 	asnJson := `{"asn": 33099, "cachegroup": 2}`

// 	reader = strings.NewReader(asnJson)

// 	request, err := http.NewRequest("POST", asnsUrl, reader)
